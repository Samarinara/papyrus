...
const LETTER_BAG = 'AAAAAAAAABBCCDDDDEEEEEEEEEEEEFFGGGHHIIIIIIIIIJKLLLLMMNNNNNNOOOOOOOOPPQRRRRRRSSSSTTTTTTUUUUVVWWXYYZ';
...
export const multiplierFor = (length: number) => length >= 9 ? 10 : length >= 7 ? 4 : length >= 4 ? 2 : 1;
export const scoreFor = (board: string[], path: number[]) => path.reduce((score, index) => score + LETTER_SCORES[board[index]], 0);
export const wordFor = (board: string[], path: number[]) => path.map(index => board[index]).join('');
export function adjacent(a: number, b: number) {
 return a !== b && Math.abs(a % 4 - b % 4) <= 1 && Math.abs(Math.floor(a / 4) - Math.floor(b / 4)) <= 1;
}
...
export function rejectionFor(word: string, dictionary: Set<string>, used: Set<string>): string | null {
